...
public class WereManager extends ClassManager{
	...
	public double damagerEvent(EntityDamageByEntityEvent event, double damage){
		...
		if(SuperNManager.worldTimeIsNight(pDamager)){
			if(SNConfigHandler.wereWeapons.contains(item.getType())){
				...
				damage += damage * snDamager.scale(SNConfigHandler.wereDamageFactor);
			}
		}
		return damage;
	}
	...
	public void killEvent(SuperNPlayer damager, SuperNPlayer victim){
		if(victim==null){
			...
			if(SNConfigHandler.wereKillSpreadCurse && !victim.isSuper() && SuperNManager.worldTimeIsNight(SupernaturalsPlugin.instance.getServer().getPlayer(victim.getName())))
			{
